<?php
    ...
    function uploadImageProducts($data, $name){
        $tmp_name = $data['tmp_name'];
        $tmp_type = $data['type'];

        /*Recortamos las imagenes a 1000x667*/
        list($img_width, $img_height) = getimagesize($tmp_name);

        $nimg_width = 1000;
        $nimg_height = 667;                        

        $new_directory  = '../assets/img/product';                       
        
        /*Funciones segun tipo de imagen*/
        ...
        if($tmp_type == 'image/jpeg'){
            ...
            $url_directory = $new_directory.'/'.$name;
            $file_origin = imagecreatefromjpeg($tmp_name);
            $file_destiny = imagecreatetruecolor($nimg_width,$nimg_height);

            imagecopyresized($file_destiny, $file_origin, 0, 0, 0, 0,$nimg_width, $nimg_height, $img_width, $img_height);                            
        }

        /*PNG*/
        else if($tmp_type == 'image/png'){

            $img_type = "PNG";

            /*Guardamos la imagen en el directorio*/
            $url_directory = $new_directory.'/'.$name;
            $file_origin = imagecreatefrompng($tmp_name);
            $file_destiny = imagecreatetruecolor($nimg_width,$nimg_height);
        ...
        /*WEBP*/
        else if($tmp_type == 'image/webp'){

            $img_type = "WEBP";

            /*Guardamos la imagen en el directorio*/
            $url_directory = $new_directory.'/'.$name;
            $file_origin = imagecreatefromwebp($tmp_name);
            $file_destiny = imagecreatetruecolor($nimg_width,$nimg_height);
        ...
        switch ($img_type) {
            case 'JPG':
                imagejpeg($file_destiny, $url_directory);
                $move = true;
                break;                        
            case 'PNG':
                imagepng($file_destiny, $url_directory);
                $move = true;
                break;
            case 'WEBP':
                imagewebp($file_destiny, $url_directory);
                $move = true;
                break;
        }
    ...
    //Delete Imagenes de productos.
    function deleteFileProducts($name){
        unlink('../assets/img/product/'.$name);
    }
    ...
    /*function sendEmail($data, $template){
        ...
        $send_email = $data['email_persona'];
        $enterprise = NAME_SENDER;
        $sender = EMAIL_SENDER;*/

        /**Enviamos el correo */
        /*$from = "MIME-Version: 1.0\r\n";
        $from .= "Content-type: text/html; charset=UTF-8\r\n";
        $from .= "From: {$enterprise} <{$sender}>\r\n";
        ob_start();
        require_once("Views/Template/Email/".$template.".php");
        $msg = ob_get_clean();
        $send = mail($send_email, $subject, $msg, $from);
